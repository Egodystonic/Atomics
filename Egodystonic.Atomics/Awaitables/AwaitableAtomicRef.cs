...
namespace Egodystonic.Atomics.Awaitables {
	...
	public class AwaitableAtomicRef<T> : IAwaitableAtomic<T> where T : class {
		...
		public AwaitableAtomicRef() : this(default) { }
		public AwaitableAtomicRef(T initialValue) : this(initialValue, RecommendedDefaultBufferSize) { }
		public AwaitableAtomicRef(T initialValue, int minBufferSize) {
			_value = new AtomicRef<T>(initialValue);
...
//		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		static bool ValueMatchesTarget(T value, T target) => TargetTypeIsEquatable ? ((IEquatable<T>)value).Equals(target) : value.Equals(target);

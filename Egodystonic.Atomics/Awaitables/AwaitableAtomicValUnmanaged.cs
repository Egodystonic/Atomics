...
namespace Egodystonic.Atomics.Awaitables {
	...
	public class AwaitableAtomicValUnmanaged<T> : IAwaitableAtomic<T> where T : unmanaged {
		...
		readonly AtomicValUnmanaged<T> _value;
		...
		public AwaitableAtomicValUnmanaged() : this(default) { }
		public AwaitableAtomicValUnmanaged(T initialValue) : this(initialValue, RecommendedDefaultBufferSize) { }
		public AwaitableAtomicValUnmanaged(T initialValue, int minBufferSize) {
			_value = new AtomicValUnmanaged<T>(initialValue);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		static unsafe bool ValueMatchesTarget(T value, T target) => *(long*)&value == *(long*)&target;
